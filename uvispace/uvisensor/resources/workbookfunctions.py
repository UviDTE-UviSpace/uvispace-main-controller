...
def read_data(process_data):
    ...
    # Open spreadsheet
    try:
        wb = openpyxl.load_workbook(process_data['filename'])
    except IOError:
...
def write_spreadsheet(process_data):
    """
    ...
    """
    name_spreadsheet = "datatemp/{}.xlsx".format(process_data['filename'])
    try:
    ...
    ws.cell('A1').value = process_data['filename']
    ...
    ws.cell('A3').value = process_data['exp_conditions']
    #Freeze header
    my_cell = ws['B7']
    ws.freeze_panes = my_cell
    #Write in spreadsheet the headboard.
    rows, cols = process_data['data_to_save'].shape
    for col in range (0, cols):
        ws.cell(column=col+1, row=6, value=process_data['header'][col])
    ...
    for row in range(0, rows):
        for col in range(0, cols):
            element = float(process_data['data_to_save'][row,col])
...
def save2master_xlsx(process_data):
    """
    ...
    folder = '\'file:///home/joselamas/UviSpace/uvispace/uvisensor/resources/datatemp/'
    name_sheet = '\'#$Sheet.'
    try:
        wb = openpyxl.load_workbook('datatemp/' + process_data['filename'] + '.xlsx')
    except:
        wb = openpyxl.Workbook()
    ws = wb.active
    ...
    while written_row:
        ...
        if element == None:
            ...
            row +=1
    avg_speed = folder + process_data['filename'] + '.xlsx' + name_sheet + 'K' + '{}'.format(row)
    row += 1
    avg_ang_speed = folder + process_data['filename'] + '.xlsx' + name_sheet + 'K' + '{}'.format(row)
    ...
    ws.cell(column=1, row=row, value=process_data['filename'])
    ws.cell(column=2, row=row, value=process_data['sp_left'])
    ws.cell(column=3, row=row, value=process_data['sp_right'])
