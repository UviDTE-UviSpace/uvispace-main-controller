...
class LocalizationNode():
    ...
    def get_image(self):
        ...
        Returns:
            image_return = image from localization node. It is equal to None
            if no image is available now.
        """
        img_return = None
        if self._img_type == ImgType.RAND:
            ...
            img_return = np.zeros([self._height, self._width], dtype = np.uint8)
            r = True
        else:
    ...
    def get_triangles(self):
        # return the last set of triangles
        if not self._triang_queue.empty():
            return self._triang_queue.get_nowait()[-1]
